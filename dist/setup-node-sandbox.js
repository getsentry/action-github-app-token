...
const {apply: localReflectApply, defineProperty: localReflectDefineProperty} =
  Reflect;

const {set: localWeakMapSet, get: localWeakMapGet} = LocalWeakMap.prototype;

const {isArray: localArrayIsArray} = Array;
...
function findBestExtensionHandler(filename) {
  ...
  for (
    let i = 0;
    (i = localStringPrototypeIndexOf(name, '.', i + 1)) !== -1;
  ...
  if (keys.length === 0)
    throw new VMError(
      `Failed to load '${filename}': Unknown type.`,
      'ELOADFAIL'
    );
...
global.setTimeout = function setTimeout(callback, delay, ...args) {
  if (typeof callback !== 'function')
    throw new LocalTypeError('"callback" argument must be a function');
...
function vmEmitArgs(event, args) {
  ...
  for (let i = 0; i < args.length; i++) {
    if (
      !localReflectDefineProperty(allargs, i + 1, {
        __proto__: null,
        value: args[i],
        writable: true,
        enumerable: true,
...
global.process = {
  ...
  listeners: function listeners(name) {
    ...
    for (let i = 0; i < all.length; i++) {
      ...
      if (h) {
        if (
          !localReflectDefineProperty(filtered, j, {
            __proto__: null,
            value: h,
            writable: true,
            enumerable: true,
