...
const thisSymbolNodeJSUtilInspectCustom = Symbol.for(
...
const thisThrowOnKeyAccess = new ThisProxy(
  emptyForzenObject,
  thisThrowOnKeyAccessHandler
);

function SafeBase() {}

if (
  !thisReflectDefineProperty(SafeBase, 'prototype', {
    __proto__: null,
...
function createBridge(otherInit, registerProxy) {
  ...
  function thisOtherHasOwnProperty(object, key) {
    ...
    try {
      return (
        otherReflectApply(otherObjectHasOwnProperty, object, [key]) === true
      );
    } catch (e) {
      // @other(unsafe)
  ...
  class BaseHandler extends SafeBase {
    ...
    doPreventExtensions(target, object, factory) {
      ...
      for (let i = 0; i < keys.length; i++) {
        ...
        if (!thisReflectDefineProperty(target, key, desc))
          throw thisUnexpected();
      }
    ...
    get(target, key, receiver) {
      ...
      switch (key) {
        ...
        case 'callee':
          if (
            typeof object === 'function' &&
            thisOtherHasOwnProperty(object, key)
          ) {
    ...
    construct(target, args, newTarget) {
      ...
      return thisFromOtherWithFactory(
        this.getFactory(),
        ret,
    ...
    getOwnPropertyDescriptorDesc(target, prop, desc) {
      ...
      if (
        desc &&
        typeof object === 'function' &&
        (prop === 'arguments' || prop === 'caller' || prop === 'callee')
      )
        desc.value = null;
      return desc;
    }
    ...
    getOwnPropertyDescriptor(target, prop) {
      ...
      if (!thisDesc.configurable) {
        ...
        if (
          !oldDesc ||
          oldDesc.configurable ||
          oldDesc.writable !== thisDesc.writable
        ) {
          if (!thisReflectDefineProperty(target, prop, thisDesc))
            throw thisUnexpected();
        }
      }
      return thisDesc;
    ...
    defineProperty(target, prop, desc) {
      ...
      try {
        ...
        if (
          otherDesc.configurable !== true &&
          (!hasBasic || !(hasFunc || hasValue))
        ) {
  ...
  class ProtectedHandler extends BaseHandler {
    ...
    set(target, key, value, receiver) {
      ...
      if (typeof value === 'function') {
        return (
          thisReflectDefineProperty(receiver, key, {
            __proto__: null,
            value: value,
            writable: true,
            enumerable: true,
    ...
    definePropertyDesc(target, prop, desc) {
      // Note: target@this(unsafe) prop@prim desc@this(safe) throws@this(unsafe)
      if (desc && (desc.set || desc.get || typeof desc.value === 'function'))
        return undefined;
      return desc;
    }
  }
  ...
  class ReadOnlyMockHandler extends ReadOnlyHandler {
    ...
    get(target, key, receiver) {
      ...
      if (
        thisReflectApply(thisObjectHasOwnProperty, mock, key) &&
        !thisOtherHasOwnProperty(object, key)
      ) {
        return mock[key];
      }
      ...
      switch (type) {
        ...
        case 'function':
          ...
          for (;;) {
            ...
            if (!proto)
              return thisProxyOther(defaultFactory, other, thisObjectPrototype);
          }
          break;
        case 'undefined':
        case 'string':
  ...
  thisAddProtoMapping(
    thisGlobalPrototypes.Object,
    otherGlobalPrototypes.Object
  );
  ...
  thisAddProtoMapping(
    thisGlobalPrototypes.VMError,
    otherGlobalPrototypes.VMError,
    'Error'
  );
