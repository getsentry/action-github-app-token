...
const {allowAsync, GeneratorFunction, AsyncFunction, AsyncGeneratorFunction} =
  data;
...
if (!localReflectSetPrototypeOf(context, localObject.prototype))
...
const LocalBuffer = fromWithFactory(
  (obj) => new BufferHandler(obj),
  host.Buffer
);
...
connect(
  localObject.prototype.__defineGetter__,
  host.Object.prototype.__defineGetter__
);
connect(
  localObject.prototype.__defineSetter__,
  host.Object.prototype.__defineSetter__
);
connect(
  localObject.prototype.__lookupGetter__,
  host.Object.prototype.__lookupGetter__
);
connect(
  localObject.prototype.__lookupSetter__,
  host.Object.prototype.__lookupSetter__
);
...
const oldPrepareStackTraceDesc = localReflectGetOwnPropertyDescriptor(
  LocalError,
  'prepareStackTrace'
);
...
if (typeof OriginalCallSite === 'function') {
  ...
  for (let i = 0; i < callSiteGetters.length; i++) {
    ...
    const funcProp = localReflectGetOwnPropertyDescriptor(
      OriginalCallSite.prototype,
      name
    );
    ...
    nameProp.value = name;
    if (!localReflectDefineProperty(func, 'name', nameProp))
      throw localUnexpected();
    funcProp.value = func;
    if (!localReflectDefineProperty(CallSite.prototype, name, funcProp))
      throw localUnexpected();
  }
  ...
  if (
    !localReflectDefineProperty(LocalError, 'prepareStackTrace', {
      ...
      set(value) {
        ...
        const wrapped = localReflectApply(
          localWeakMapGet,
        ...
        localReflectApply(localWeakMapSet, wrappedPrepareStackTrace, [
          value,
          newWrapped,
        ]);
        localReflectApply(localWeakMapSet, wrappedPrepareStackTrace, [
  ...
  localReflectDefineProperty(
    LocalError,
    'prepareStackTrace',
    oldPrepareStackTraceDesc
  );
} else {
...
function overrideWithProxy(obj, prop, value, handler) {
  ...
  if (!localReflectDefineProperty(obj, prop, {__proto__: null, value: proxy}))
    throw localUnexpected();
  return proxy;
}
...
if (GeneratorFunction) {
  if (!localReflectSetPrototypeOf(GeneratorFunction, proxiedFunction))
    throw localUnexpected();
  overrideWithProxy(
    GeneratorFunction.prototype,
    'constructor',
    GeneratorFunction,
    makeCheckFunction(false, true)
  );
}
if (AsyncFunction) {
  if (!localReflectSetPrototypeOf(AsyncFunction, proxiedFunction))
    throw localUnexpected();
  overrideWithProxy(
    AsyncFunction.prototype,
    'constructor',
    AsyncFunction,
    makeCheckFunction(true, false)
  );
}
if (AsyncGeneratorFunction) {
  if (!localReflectSetPrototypeOf(AsyncGeneratorFunction, proxiedFunction))
    throw localUnexpected();
  overrideWithProxy(
